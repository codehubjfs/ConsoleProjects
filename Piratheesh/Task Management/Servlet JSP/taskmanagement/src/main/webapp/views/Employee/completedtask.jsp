...
<body>
            ...
            <nav id="sidebar" class="col-md-2 d-none d-md-block sidebar">
                <div class="sidebar-sticky">
                    ...
                    <ul class="nav flex-column">
                        ...
                        <li class="nav-item">
                            <a href="${pageContext.request.contextPath}/CompletedTask" class="nav-link active">Completed Task</a>
                        </li>
                        <li class="nav-item">
                            <a href="${pageContext.request.contextPath}/OverlayedTask" class="nav-link">Over Due Task</a>
                        </li>
                        <li class="nav-item">
                            <a href="${pageContext.request.contextPath}/views/Employee/calender.jsp" class="nav-link">Personal Task Management</a>
                        </li>
                        <li class="nav-item">
            ...
            <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
                ...
                <div id="navigation">
                    <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="${pageContext.request.contextPath}/EmployeeDashBoard">Dashboard</a></li>
    ...
    <tbody>
        ...
        <c:forEach var="task" items="${completedTasks}">
            <tr>
                ...
                <td>${task.status}</td>
                <td>
                    <a href="#" class="info-task" data-task-id="${task.task_id}" data-task-name="${task.task_name}" data-task-start="${task.start_date}" data-task-end="${task.end_date}" data-task-status="${task.status}" data-task-desp="${task.desp}" data-task-priority="${task.priority}" data-toggle="modal" data-target="#aboutTask">
        ...
        <div class="modal fade" id="aboutTask" tabindex="-1" role="dialog" aria-labelledby="aboutTaskLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <c:forEach var="task" items="${completedTasks}">
                        <p><strong>Description: </strong><span id="task-desp">${task.desp}</span></p>
                        <p><strong>Priority: </strong><span id="task-priority">${task.priority}</span></p>
                        <p><strong>Document: </strong>
                            <div>
                                <a href="path/to/your/document.pdf" class="btn btn-primary" download>Download PDF</a>
                            </div>
                        </p>
                       </c:forEach>
                    </div>
    ...
    $(document).ready(function () {
        $('.info-task').on('click', function () {
            var taskDesp = $(this).data('task-desp');
            var taskPriority = $(this).data('task-priority');
            var taskDocument = "path/to/your/document.pdf"; // Update this if the document path is dynamic

            $('#task-desp').text(taskDesp);
            $('#task-priority').text(taskPriority);
            $('#task-document').attr('href', taskDocument);
        });
    });
	</script>
</body>
