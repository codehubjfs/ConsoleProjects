...
public class PendingDAO {
	...
	private static final String PENDING_TASK_QUERY = "SELECT e.name, t.task_id, t.task_name, t.task_desp, t.start_date, t.end_date, t.task_priortiy ,st.status " +
            "FROM employee e " +
            "JOIN transcation ts ON e.emp_id = ts.emp_id " +
            "JOIN task t ON t.task_id = ts.task_id " +
            "JOIN task_status st ON st.id = ts.status_id " +
            "WHERE e.email = ? AND st.id = 3";
	
	public List<PendingBeans> getPendingTask(String email){
		
		List<PendingBeans> pendingTasks = new ArrayList<>();
		
		try {
			Connection connection = DBConnection.getInstance().getConnection();
			PreparedStatement stmt = connection.prepareStatement(PENDING_TASK_QUERY);
			stmt.setString(1, email);
			ResultSet rs = stmt.executeQuery();
			
			while(rs.next()) {
				PendingBeans task = new PendingBeans();
