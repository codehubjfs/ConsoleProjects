...
public class TaskManager {
	...
	public static void addTask(Scanner sc) {
		...
		try {
			...
			System.out.println("Do you want to add this task? (yes/no): ");
	...
	private static String TaskPriCheck(String priority, Scanner sc) {
		...
		while (flag) {
			...
			priority = sc.next().toLowerCase().trim();
	...
	public static boolean taskExists(int id) {
		...
		try {
			Connection conn = DbmsConnection.getInstance().getConnection();
			PreparedStatement stmt = conn.prepareStatement("SELECT 1 FROM task WHERE task_id = ?");
			stmt.setInt(1, id);
			ResultSet rs = stmt.executeQuery();
			exists = rs.next();
		} catch (SQLException e) {
...
//	public static boolean taskExistsemp(int taskid, int id, int empid) {
//	    boolean exists = false;
//	    try {
//	        Connection conn = DbmsConnection.getInstance().getConnection();
//	        PreparedStatement stmt = conn.prepareStatement(
//	            "SELECT count(*) FROM employee e JOIN transaction t ON t.emp_id = e.emp_id WHERE e.emp_id = ? AND e.mag_id = ? AND t.task_id = ?");
//	        stmt.setInt(1, id);
//	        stmt.setInt(2, empid);
...
//	}
	...
	public static void DisplayEmployee(String mail, int magid) {
		...
		try {
			PreparedStatement stmt = DbmsConnection.getInstance().getConnection().prepareStatement(
					" select e.emp_id,e.name,ts.task_id from employee e join transcation t on t.emp_id=e.emp_id join task ts on ts.task_id=t.task_id where e.mag_id=? and e.role='employee'");
			stmt.setInt(1, magid);
