...
public class Admin {
	...
	public static int optCheck(int opt1, Scanner sc) {
		...
		while (inputflag) {
			try {
				...
				opt1 = Integer.parseInt(sc.next()); // Read and parse input as an integer
				System.out.println();

				// Check if the entered choice is either 1 or 2
				if (opt1 != 1 && opt1 != 2) {
					// If not, throw a custom exception
					throw new InvalidNumberException("Please enter a valid number");
				}
				// If valid input is received, set flag to false to exit loop
				inputflag = false;
				...
				// Handle case where input is not 1 or 2
				System.out.println("!" + e.getMessage() + "!");
			}
		}
		return opt1; // Return the valid input choice
	}
	...
	public static void showeveEmp() {
		try {
			PreparedStatement stmt = DbmsConnection.getInstance().getConnection()
					.prepareStatement("select * from employee order by emp_id");
			...
			System.out.println(
					"|-----|--------------------|---------------------------------|-----------|--------|--------------|--------------|------------|");
			System.out.println(
					"| ID  |        Name        |               Mail              |    Role   | Gender | Phone Number | City         |  Hiredate  |");
			System.out.println(
					"|-----|--------------------|---------------------------------|-----------|--------|--------------|--------------|------------|");
	...
	public static void showemp() {
		try {
			...
			PreparedStatement stmt = DbmsConnection.getInstance().getConnection().prepareStatement(sql);
			ResultSet rs = stmt.executeQuery();
			...
			System.out.println(
					"|-----|--------------------|---------------------------------|--------|--------------|--------------|------------|");
			System.out.println(
					"| ID  |        Name        |               Mail              | Gender | Phone Number | City         |  Hiredate  |");
			System.out.println(
					"|-----|--------------------|---------------------------------|--------|--------------|--------------|------------|");
	...
	public static void showmag() {
		...
		managers.stream().forEach(manager -> {
			System.out.printf("| %-4s| %-19s| %-32s| %-7s| %-13s| %-13s| %-11s|%n", manager.get("ID"),
					manager.get("Name"), manager.get("Mail"), manager.get("Gender"), manager.get("Phone Number"),
					manager.get("City"), manager.get("HireDate"));
		});
		System.out.println(
				"|-----|--------------------|---------------------------------|--------|--------------|--------------|------------|");
	}
