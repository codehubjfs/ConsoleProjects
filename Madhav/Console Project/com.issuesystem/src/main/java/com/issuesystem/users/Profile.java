...
public class Profile {
	...
	//Edit the mail id for student,Warden
	public static void editMail(Scanner sc,String username,int role) throws  SQLException {
		...
		if(role==1) {
			...
			flag=true;
			if(flag) {
			String sql="update students set mailid=? where username=?";
		...
		if(role==2) {
			...
			flag=true;
			if(flag) {
			String sql="update warden set mail_id=? where username=?";
			PreparedStatement stmt=DBConnection.getDBConnextion().prepareStatement(sql);
			stmt.setString(1, mail);
			stmt.setString(2,username);
			stmt.executeUpdate();
		...
		//Edit the name for Student,Warden
		...
		public static void editName(Scanner sc,String username,int role) throws SQLException, InvalidInputException {
			...
			if(role==1) {
				...
				while(flag) {
					...
					flag=true;
					if(flag) {
					String sql="update students set name=? where username=?";
					PreparedStatement stmt=DBConnection.getDBConnextion().prepareStatement(sql);
			...
			if(role==2) {
				...
				while(flag) {
					...
					flag=true;
					if(flag) {
					String sql="update warden set name=? where username=?";
					PreparedStatement stmt=DBConnection.getDBConnextion().prepareStatement(sql);
					stmt.setString(1, name);
					stmt.setString(2,username);
					stmt.executeUpdate();
		...
		public static void validateInput(String input) throws InvalidInputException {
		    ...
		    if (!input.matches("[a-zA-Z]+")) {
		        throw new InvalidInputException("!!!!!Input must contain only letters (no numbers and symbols and spaces)!!!!!");
		    }
		}
