...
public class ViewIssue {
	...
	public static void toViewIssue(Scanner sc, String username) throws SQLException {
		...
		String sql="select * from issues where raised_by=?";
        ...
        if(idFlag) {
    		...
    		String sql1="select * from issues where raised_by=?";
    		...
    		while(rs1.next()) {
        	 ...
        	 issueList.forEach(issue -> {
        		 ...
        		 System.out.printf("%-10s%-30s%-75s%-55s%-25s%-20s%-25s%-20s%-20s%-20s%n",
     		    				 issue.getIssueId(),issue.getIssueTitle(), issue.getDescription(),
     		    				 issue.getTicketRaisedDate(), issue.getCategoryId(), issue.getRaisedBy(),
	     		    			 issue.getAllocatedTo(), issue.getPriority(), 
