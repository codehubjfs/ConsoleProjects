...
<head>
  ...
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
    ...
    .faded {
        background-color: #f0f0f0; /* Light gray color for fade effect */
    }
</style>
	 <% 
  ...
  <div class="modal-dialog">
		  ...
		  <div class="modal-dialog" role="document">
		    <div class="modal-content">
		      ...
		      <div class="modal-footer">
		        ...
		        <button type="button" class="btn btn-outline-danger" id="confirmCancelButton">Yes</button>
		      </div>
		    </div>
		  </div>
		</div>
    <div class="container">
	    ...
	    <c:if test="${not empty bookList}">
	        <c:forEach var="book" items="${bookList}" varStatus="status">
	            <div class="card <c:if test='${book.bookStatus == "CANCELLED"}'>faded</c:if>'">
	                <div class="innerCard row">
	                    <div class="image-container col-3">
	                        <a href="#"><img src="<%= request.getContextPath() %>/assert/image/hallcard-1.jpg" alt="hall"></a>
	                    </div>
	                    <div class="content col-8">
	                        <div class="hall-content d-flex">
	                            <div class="hall_name pe-4 text-center"><strong>${book.hall.hallName}</strong></div>
	                            <div class="hall_location pe-4 ">${book.hall.location}</div> 
	                            <div class="price pe-4 "><p class="text-center mt-3"><strong>Per day price :</strong>&#8377; ${book.hall.price}</p></div>
	               
	                        </div>
	 
	                        <div class="booking-content d-flex">
	                            <div class="book-c flex-grow-0 flex-shrink-0 bookeduration pe-4 text-center"><strong>Start Date :</strong>${book.startDate}</div>
	                            <c:if test="${book.bookStatus == 'PENDING'}">
							      <h4 class="d-inline me-4">Request state :<p style="color:orange;" class="d-inline ms-2">Pending</p></h4>
								</c:if>
								 <c:if test="${book.bookStatus == 'CANCELED'}">
							      <h4 class="d-inline me-4">Request state :<p style="color:red;" class="d-inline ms-2">Canceled</p></h4>
								</c:if>
								<c:if test="${book.bookStatus == 'CONFIRMED'}">
							      <h4 class="d-inline me-4">Request state :<p style="color:green;" class="d-inline ms-2">Confrimed</p></h4>
								</c:if>
	                            <c:choose>
	                                <c:when test="${book.bookStatus == 'APPROVED'}">
	                                  <form action="<%= request.getContextPath() %>/Payment" method="get">
			                                  <input value="${book.bookingId}" name="bookingID" style="display:none"/>
			                                    <div class="book-c flex-grow-0 flex-shrink-0 mt-2 pe-4 align-items-center">
			                                        <button class="btn btn-outline-success btn-block" type="submit">Confirm booking</button>
			                                    </div>
	                                    </form>
	                                    </c:when>
	                                </c:choose>
	                                 <c:choose>
	                                <c:when test="${book.bookStatus == 'PENDING' || book.bookStatus == 'APPROVED'}">
		                                <form action="<%= request.getContextPath() %>/cancelRequest" method="get" id="cancelForm${book.bookingId}">
				                                <input value="${book.bookingId}" name="bookingID" style="display:none"/>
				                                <div class="book-c flex-grow-0 flex-shrink-0 mt-2 pe-4 align-items-center">
				                                    <button type="button" class="btn btn-outline-danger btn-block" onclick="showConfirmModal(${book.bookingId})">Cancel booking</button>
				                                </div>
	                             		</form>
	                                </c:when>
	                            </c:choose>
	...
	<script>
      ...
      function showConfirmModal(bookingId) {
    	    $('#confirmCancelModal').data('bookingId', bookingId).modal('show');
    	}

    	document.getElementById('confirmCancelButton').addEventListener('click', function () {
    	    var bookingId = $('#confirmCancelModal').data('bookingId');
