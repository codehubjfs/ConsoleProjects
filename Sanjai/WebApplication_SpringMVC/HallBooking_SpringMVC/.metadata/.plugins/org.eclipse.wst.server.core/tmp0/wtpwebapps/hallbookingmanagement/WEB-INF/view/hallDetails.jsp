...
<div class="modal" id="logoutModal" tabindex="-1" aria-labelledby="logoutModalLabel" aria-hidden="true">
 <form action="logout" method="get">
  ...
  <form action="viewLogin" method="get">
	    <div class="modal-content">
		      <div class="modal-header">
		        <h5 class="modal-title">Booking Required</h5>
		        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
		      </div>
		      <div class="modal-body">
		        <p>If you want to continue booking, please log in.</p>
		      </div>
		      <div class="modal-footer">
   ...
   <header>
        ...
        <div class="row mt-4">
           ...
           <div class="col-md-6">
		    <div class="form-container">
		        ...
		        <form class="row g-3 needs-validation" novalidate action="RequestSelectHall" method="get" id="requestForm">
		            <div class="col-md-6">
		                <label for="name" class="form-label">Name <sup style="color:red">*</sup></label>
		                <input type="text" class="form-control" id="name" name="name" placeholder="Enter Name">
		                <div class="invalid-feedback" id="invalidName" style="color:red;">Please enter your name.</div>
		            </div>
		            <div class="col-md-6">
		                <label for="mobile" class="form-label">Mobile <sup style="color:red">*</sup></label>
		                <div class="input-group">
		                    <span class="input-group-text">+91</span>
		                    <input type="text" class="form-control" id="mobile" name="mobile" placeholder="Mobile Number"  oninput="validatePhone()" required pattern="\d{10}">
		                    <div class="invalid-feedback" id="invalidmobile">Please enter a valid 10-digit mobile number.</div>
		                </div>
		            </div>
		            <div class="col-md-6">
		                <label for="function" class="form-label">Function <sup style="color:red">*</sup></label>
		                <select class="form-control" id="function" name="event"  oninput="validateName()" required>
		            ...
		            <div class="col-md-6">
		                ...
		                <select class="form-control form-select" id="seating"  oninput="validateSeat()" name="seat" required>
		                    <c:forEach items="${hallDetail.seat}" var="seat">
		                        <option value="${seat.arrangementType}">${seat.arrangementType}</option>
		                    </c:forEach>
		                </select>
		                <div class="invalid-feedback">Please select a seating arrangement.</div>
		            </div>                        
		            <div class="col-md-6">
		                <label for="date" class="form-label">Date <sup style="color:red">*</sup></label>
		                <input type="date" class="form-control" id="date" min=""  name="bookedDate" required>
		                <div class="invalid-feedback" id="checkDate">Invalid date. Please enter a date between today and one year from today.</div>
		            </div>
		            <div class="col-md-6">
		                <label for="numberDays" class="form-label">Number of Days <sup style="color:red">*</sup></label>
		                <input type="number" class="form-control" id="numberDays" min="1" placeholder="No of Days.." name="numberDay"  required>
		                <div class="invalid-feedback" id="invalidnoOfDays">Please enter the number of days.</div>
		            </div>
		            <div class="col-md-6">
		                <button class="btn btn-success btn-block mt-2" id="sub-btn" type="button" onclick="checkSession()" disabled>REQUEST NOW</button>
		            </div>
		        </form>
		    </div>
		</div>
    ...
    <div class="container top-features">
        ...
        <div class="row">
            <div class="col-md-3 col-sm-6">
                <div class="feature-card text-center">
                    ...
                    <h3>Hall Capacity</h3>
                    <p>upto ${hallDetail.capacity}</p>
                </div>
            </div>
    
            <div class="col-md-3 col-sm-6" style="height:100px;">
                <div class="feature-card text-center">
                    <div class="d-block">
                    ...
                    <div class="btn-group dropright">
                       <select aria-label="Default select example">
						    <option selected disabled>Available Seat</option>
						    <c:forEach items="${hallDetail.seat}" var="seat">
						        <option disabled>${seat.arrangementType} → ${seat.capacity}</option>
						    </c:forEach>
						</select>
                    </div>
                </div>
            ...
            <div class="col-md-3 col-sm-6">
                <div class="feature-card text-center">
                  ...
                  <c:choose>
				    <c:when test="${halldetail.isAcHall}">
				        <p>Yes</p>
				    </c:when>
				    <c:otherwise>
				        <p>No</p>
            ...
            <div class="col-md-3 col-sm-6">
                <div class="feature-card text-center">
                    <img src="${pageContext.request.contextPath}/assert/image/sound-system.png" width="30px" alt="Lift Access to Hall">
                    <h3>Sound-System</h3>
                    <c:forEach var="amenity" items="${hallDetail.amenities}">
	                     <c:if test="${amenity.amenityType == 'Sound system'}">
	                   		 <c:set var="soundSystem" value="true" />
	                   	</c:if>
                   	</c:forEach>
                   	 <c:choose>
		                <c:when test="${soundSystem}">
		                    <p>Yes</p>
		                </c:when>
		                <c:otherwise>
		                    <p>No</p>
        ...
        <div class="row">
            <div class="col-md-3 col-sm-6">
                <div class="feature-card text-center">
                    <img src="${pageContext.request.contextPath}/assert/image/bar.png" width="30px" alt="Rooms">
                    <h3>Bar</h3>
                    <c:forEach var="amenity" items="${hallDetail.amenities}">
	                     <c:if test="${amenity.amenityType == 'Bar setup'}">
	                   		 <c:set var="Barsetup" value="true" />
	                   	</c:if>
                   	</c:forEach>
                   	 <c:choose>
		                <c:when test="${Barsetup}">
		                    <p>Yes</p>
		                </c:when>
		                <c:otherwise>
		                    <p>No</p>
            ...
            <div class="col-md-3 col-sm-6">
                <div class="feature-card text-center">
                    ...
                    <h3>Catring Facility</h3>
                     <c:forEach var="amenity" items="${hallDetail.amenities}">
	                     <c:if test="${amenity.amenityType == 'Catering'}">
	                   		 <c:set var="Catering" value="true" />
	                   	</c:if>
                   	</c:forEach>
                   	 <c:choose>
		                <c:when test="${Catering}">
		                    <p>Yes</p>
		                </c:when>
		                <c:otherwise>
		                    <p>No</p>
            ...
            <div class="col-md-3 col-sm-6">
                <div class="feature-card text-center">
                    ...
                    <h3>Car Parking</h3>
                    <c:forEach var="amenity" items="${hallDetail.amenities}">
	                     <c:if test="${amenity.amenityType == 'Parking'}">
	                   		 <c:set var="Parking" value="true" />
	                   	</c:if>
                   	</c:forEach>
                   	 <c:choose>
		                <c:when test="${Parking}">
            ...
            <div class="col-md-3 col-sm-6">
                <div class="feature-card text-center">
                    <img src="${pageContext.request.contextPath}/assert/image/ice-cream.png" width="30px" alt="Car Parking">
                    <h3>Ice-Cream-Bar</h3>
                    <c:forEach var="amenity" items="${hallDetail.amenities}">
	                     <c:if test="${amenity.amenityType == 'Ice-cream Bar'}">
	                   		 <c:set var="IcecreamBar" value="true" />
	                   	</c:if>
                   	</c:forEach>
                   	 <c:choose>
		                <c:when test="${IcecreamBar}">
		                    <p>Yes</p>
		                </c:when>
		                <c:otherwise>
		                    <p>No</p>
      ...
      <div class="container text-center text-md-start mt-5">
        ...
        <div class="row mt-3">
          ...
          <div class="col-md-4 col-lg-3 col-xl-3 mx-auto mb-md-0 mb-4">
            ...
            <p><i class="fas fa-home me-3"></i> Singanalur, Coimbatore</p>
            <p>
  ...
  <script>
 	...
 	document.getElementById("name").addEventListener("input", function() {
 	    ...
 	    if (!regexPattern.test(checkName)) {
 	...
 	document.getElementById("mobile").addEventListener("input", function() {
 	    ...
 	    if (!regexPattern.test(checkMobile)) {
 	...
 	document.getElementById("date").addEventListener("input", function() {
 	    var bookDate = document.getElementById("date").value;
 	    var enteredDate = new Date(bookDate);
 	...
 	document.getElementById("numberDays").addEventListener("input", function() {
 	    var regexPattern = /^[0-9]+$/;
 	    var checkDays = document.getElementById("numberDays").value;
 	    if (!regexPattern.test(checkDays)) {
 	        document.getElementById("invalidnoOfDays").style.display = "block";
 	        dayCheck = false;
 	    } else {
 	        document.getElementById("invalidnoOfDays").style.display = "none";
 	...
 	function validateForm() {
 	    if (dateCheck && nameCheck && phoneCheck && dayCheck) {
      ...
      document.getElementById('confirmLogoutButton').addEventListener('click', function() {
        ...
        document.getElementById('logout').addEventListener('click', function(event) { event.preventDefault();
