...
public class Seats {
    ...
    public Seats(int seatId, String arrangementType) {
        this.seatId = seatId;
        this.arrangementType = arrangementType;
    }
    ...
    public  List<Seats> arrangementList() throws SQLException {
        String query = "Select * from Seating_Arrangement ";
        PreparedStatement statement = DBConnection.getConnection().prepareStatement(query);
        ResultSet set = statement.executeQuery();
        List<Seats> seatOrderList = new ArrayList<>();
        while(set.next()){
            arrangementType=set.getString("arrangement_type");
            seatId =set.getInt("ARRANGEMENT_ID");
            seatOrderList.add(new Seats(seatId,arrangementType));
        }
