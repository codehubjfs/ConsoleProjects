...
public class Authentication {
    ...
    public <T extends Person> T login(AccountType accountType) {
        try {
            ...
            do {
                if (attempt) {
                    // Prompt user to continue or exit after a failed attempt
                ...
                // Check if the username exists in the account map
                if (accounts.containsKey(username)) {
                    ...
                    if (account.getPassword().equals(password)) {
                        ...
                        if (account.getStatus().equals(AccountStatus.BLOCKED)) {
                            ...
                            System.out.print("\n+-------------------------------------------------------------------------+");
                            System.out.print("\n|  Welcome back! You've successfully logged in. Let's make today awesome! |");
                            System.out.print("\n+-------------------------------------------------------------------------+\n\n");
                        }
                        ...
                        try (PreparedStatement statement = DBConnection.getConnection().prepareStatement(query)) {
                            ...
                            try (ResultSet set = statement.executeQuery()) {
                                if (set.next()) {
                                    isContains = true;
                                    // Initialize Admin or Customer based on account type
                ...
                // Exit if maximum number of attempts is exceeded
                if (count == 4) {
    ...
    public <T extends Person> T register(AccountType accountType) {
        ...
        try {
            boolean flag = false;
            // Prompt user to enter full name and validate
            do {
                try {
            ...
            flag = false;
            Gender gender = null;
            // Prompt user to enter gender and validate
            do {
            ...
            String email = "";
            // Prompt user to enter email and check for existence
            do {
                try {
            ...
            String phoneNumber = "";
            // Prompt user to enter phone number and check for existence
            do {
                try {
            ...
            // Prompt user to enter optional door number and validate length
            while (true) {
                System.out.print("Enter Your Door Number (under 10 character)[optional]");// Entering the door number is optional where door number =null;
                doorNumber = sc.readLine().trim();
                if (doorNumber.length() > 10) {
                    System.out.println("Invalid Input- Enter the character under 10 character");
            ...
            if (!isAddressing) {
                // Prompt user to enter street name, town name, city, and pincode
                System.out.print("Enter Your Street Name   ");
                streetName = sc.readLine().trim();
                boolean townFlag = false;
                do {
                    try {
                        System.out.print("Enter Your Town Name(under 30 characters)");
                        townName = Validate.validateName(sc.readLine().trim());
                        if (townName.length() > 30) {
                            System.out.println("The town name contains at most 30 characters");
                            continue;
                        }
                        break;
                ...
                do {
                    try {
                        System.out.print("Enter Your City (at most 30 characters are allowed )  ");
                        city = Validate.validateName(sc.readLine().trim());
                        if (city.length() > 30) {
                            System.out.println("Invalid Input at most 30 characters allowed");
                            continue;
                        }
                        break;
                    } catch (FullNameException | IOException e) {
            ...
            // Prompt user to enter username and validate uniqueness
            do {
                try {
            ...
            // Prompt user to enter and validate password
            do {
            ...
            // Register admin or customer based on account type
            if (accountType.equals(AccountType.ADMIN)) {
                ...
                statement.setString(10, account.getPassword());
                if (statement.executeUpdate() > 0) {
                    System.out.println(" Congratulations! You've registered successfully. Welcome aboard!");
                    return (T) customer;
                }
            }
