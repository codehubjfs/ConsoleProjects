package com.jobportal.controler;

import jakarta.servlet.ServletException;
import jakarta.servlet.annotation.WebServlet;
import jakarta.servlet.http.HttpServlet;
import jakarta.servlet.http.HttpServletRequest;
import jakarta.servlet.http.HttpServletResponse;
import java.io.IOException;

import com.jobportal.bean.Employer;
import com.jobportal.bean.JobSeekers;
import com.jobportal.dao.EmployerDao;
import com.jobportal.dao.JobsSeekersDao;

/**
 * Servlet implementation class AdminSeekerEdit
 */
public class AdminSeekerEdit extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AdminSeekerEdit() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		response.getWriter().append("Served at: ").append(request.getContextPath());
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		doGet(request, response);
		
		
		String email =request.getParameter("email");
		String status =request.getParameter("status");
		System.out.println("email"+email);
		Employer js  = new Employer();
		js.setStatus(status);
		js.setEmail(email);
			EmployerDao jd = new EmployerDao();
		System.out.println("welcome edite");
			try {
				boolean value =jd.updateStatus(email, status);
				 response.sendRedirect("AdminEmployerView");
				
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			
	}

}
